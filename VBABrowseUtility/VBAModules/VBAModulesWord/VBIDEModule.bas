...
Initialize:
  ...
  'open Excel VBIDE to gegt procedure
...
Main:
  'parse selected item for procedure name
  On Error Resume Next
  ...
  With ControlPanel.ProcedureListBox
    For I = 0 To .ListCount - 1
      If .Selected(I) Then
        ...
        'call sub to show selected Excel procedure in Word
        Call ExportVBComponent(VBComp, folderName, ProcName, fileName)
        Exit Sub
      End If
    Next I
  End With
